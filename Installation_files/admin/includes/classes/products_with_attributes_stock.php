...
class products_with_attributes_stock extends base {
  ...
  function get_products_attributes($products_id, $languageId = 1)
  {
    ...
    $sql = "SELECT DISTINCT popt.products_options_id, popt.products_options_name, popt.products_options_sort_order, popt.products_options_type
            ...
            AND popt.language_id = " . (int)$languageId . "
            " . $options_order_by;
    ...
    if ($attributes->RecordCount() > 0) {
      ...
      foreach ($attributes as $attribute) {
        ...
        $sql = "SELECT pov.products_options_values_id, pov.products_options_values_name,
                ...
                AND pa.options_id = " . (int)$attribute['products_options_id'] . "
        ...
        foreach ($attributes_array_ans as $item) {
          $attributes_array[$attribute['products_options_name']][] = array(
            'id' => $item['products_attributes_id'],
            'text' => $item['products_options_values_name'] . ' (' . $item['price_prefix'] . '$' . zen_round($item['options_values_price'], 2) . ')',
            'display_only' => $item['attributes_display_only']
          );
        }
      }
  ...
  function update_parent_products_stock($products_id)
  {
    ...
    $query = "SELECT SUM(quantity) AS quantity, products_id
              FROM " . TABLE_PRODUCTS_WITH_ATTRIBUTES_STOCK . "
              WHERE products_id = " . (int)$products_id;
    $quantity = $db->Execute($query);

    $updateQuery = "UPDATE " . TABLE_PRODUCTS . "
    ...
    if ($quantity->RecordCount() > 0 && $quantity->fields['products_id'] == zen_get_prid($products_id)) {
      ...
      $query2 = "SELECT p.products_quantity AS quantity
      ...
      if ($quantity_orig->RecordCount() > 0 && true /* This is where a switch could be introduced to allow setting to 0 when synchronizing with the SBA table. But as long as true, and the item is not tracked by SBA, then there is no change in the quantity.  header message probably should also appear.. */) {
        $updateQuery = $db->bindVars($updateQuery, ':quantity:', $quantity_orig->fields['quantity'], 'float');
      } else {
        $updateQuery = $db->bindVars($updateQuery, ':quantity:', 0, 'float');
      }
    }
  ...
  function update_all_parent_products_stock()
  {
    ...
    foreach ($products_array as $products_id) {
      ...
      //  turned off, the value will stay the same otherwise, it would be set to zero.
      if ($quantity->RecordCount() > 0 && $quantity->fields['products_id'] == zen_get_prid($products_id)) {
        $updateQuery = $db->bindVars($updateQuery, ':quantity:', $quantity->fields['quantity'], 'float');
      } else {
        // Should add a switch to allow not resetting the quantity to zero when synchronizing quantities... This doesn't entirely make sense that because the product is not listed in the SBA table, that it should be zero'd out...
        $query2 = "SELECT p.products_quantity AS quantity
                   FROM " . TABLE_PRODUCTS . " p
  ...
  function get_products_with_attributes()
  {
    ...
    $query = "SELECT DISTINCT pa.products_id, d.products_name, p.products_quantity, p.products_model, p.products_image
              FROM " . TABLE_PRODUCTS_ATTRIBUTES . " pa
              LEFT JOIN " . TABLE_PRODUCTS_DESCRIPTION . " d ON d.products_id = pa.products_id
  ...
  function displayFilteredRows($SearchBoxOnly = null, $NumberRecordsShown = null, $ReturnedProductID = null)
  {
    ...
    $query_products = "SELECT DISTINCT pa.products_id,
                                       d.products_name,
                                       p.products_quantity, p.products_model, p.products_image, p.products_type, p.master_categories_id
                       FROM " . TABLE_PRODUCTS_ATTRIBUTES . " pa,
    ...
    $html = '';
    if (!isset($_GET['seachPID']) && !isset($_GET['pwas-search-button']) && !isset($_GET['updateReturnedPID'])) {
      $html .= '<table class="table pageResults">';
      $html .= '<tr>';
      $html .= '<td>' . $products_split->display_count($products_query_numrows, STOCK_SET_SBA_NUMRECORDS, $_GET['page'], TEXT_DISPLAY_NUMBER_OF_PRODUCTS) . '</td>';
      $html .= '<td class="text-right">' . $products_split->display_links($products_query_numrows, STOCK_SET_SBA_NUMRECORDS, MAX_DISPLAY_PAGE_LINKS, $_GET['page']) . '</td>';
      $html .= '</tr>';
      $html .= '</table>';
    }
    $html .= '<div class="row">';
    ...
    $html .= '<div class="form-group"><button type="submit" class="btn btn-primary btn-sm">' . IMAGE_SAVE . '</button><span class="help-block">Hint: To quickly edit click in the "Quantity in Stock" field.</span></div>';
    $html .= '</div>';
    $html .= '<table id="mainProductTable" class="table table-bordered">';
    $html .= '<thead>';
    $html .= '<tr>';
    $html .= '<th class="thProdId">' . PWA_PRODUCT_ID . '</th>';
    $html .= '<th class="thProdName">' . PWA_PRODUCT_NAME . '</th>';
    ...
    $html .= '<th class="thProdQty">' . PWA_QUANTITY_FOR_ALL_VARIANTS . '</th>';
    $html .= '<th class="thProdAdd">' . PWA_ADD_QUANTITY . '</th>';
    $html .= '<th class="thProdSync">' . PWA_SYNC_QUANTITY . '</th>';
    $html .= '</tr>';
    $html .= '</thead>';
    $html .= '<tbody>' . "\n";
    ...
    foreach ($products as $product) {
      ...
      if ($_SESSION['pwas_class2']->zen_product_is_sba($product['products_id'])) {
        if ($product['products_quantity'] > $attribute_quantity->fields['total_quantity']) {
          $synchronized = '<br> Prod Qty > Attrib Qty ';
        } elseif ($product['products_quantity'] != $attribute_quantity->fields['total_quantity']) {
          $synchronized = '<br> Prod Qty < Attrib Qty ';
        }
      }

      $html .= '<tr class="productRow">' . "\n";
      $html .= '<td class="tdProdId">' . $product['products_id'] . '</td>';
      $html .= '<td class="tdProdName">' . $product['products_name'] . '</td>';

      if (STOCK_SHOW_IMAGE == 'true') {
        $html .= '<td class="tdProdImage">' . zen_info_image(zen_output_string($product['products_image']), zen_output_string($product['products_name']), "60", "60") . '</td>';
      }

      $html .= '<td class="tdProdModel">' . $product['products_model'] . '<br><a href="' . zen_href_link(FILENAME_PRODUCT, "page=1&amp;product_type=" . $product['products_type'] . "&amp;cPath=" . $product['master_categories_id'] . "&amp;pID=" . $product['products_id'] . "&amp;action=new_product", 'NONSSL') . '">Link</a><br><br><a href="' . zen_href_link(FILENAME_ATTRIBUTES_CONTROLLER, "products_filter=&amp;products_filter=" . $product['products_id'] . "&amp;current_category_id=" . $product['master_categories_id'], 'NONSSL') . '">' . BOX_CATALOG_CATEGORIES_ATTRIBUTES_CONTROLLER . '</a></td>';
      $html .= '<td class="tdProdQty">' . $product['products_quantity'] . $synchronized . '</td>';
      $html .= '<td class="tdProdAdd"><a href="' . zen_href_link(FILENAME_PRODUCTS_WITH_ATTRIBUTES_STOCK, "action=add&amp;products_id=" . $product['products_id'] . '&amp;search_order_by=' . $search_order_by, 'NONSSL') . '">' . PWA_ADD_QUANTITY . '</a><br><br><a href="' . zen_href_link(FILENAME_PRODUCTS_WITH_ATTRIBUTES_STOCK, "action=delete_all&amp;products_id=" . $product['products_id'] . '&amp;search_order_by=' . $search_order_by, 'NONSSL') . '">' . PWA_DELETE_VARIANT_ALL . '</a></td>';
      $html .= '<td class="tdProdSync"><a href="' . zen_href_link(FILENAME_PRODUCTS_WITH_ATTRIBUTES_STOCK, "action=resync&amp;products_id=" . $product['products_id'] . '&amp;search_order_by=' . $search_order_by, 'NONSSL') . '">' . PWA_SYNC_QUANTITY . '</a></td>';
      $html .= '</tr>' . "\n";
      // SUB
      if ($attribute_products->RecordCount() > 0) {

        $html .= '<tr class="productGroup">';
        $html .= '<td colspan="7">';
        $html .= '<table class="table stockAttributesTable">';
        $html .= '<thead>';
        $html .= '<tr>';
        $html .= '<th class="stockAttributesHeadingStockId">' . PWA_STOCK_ID . '</th>';
        $html .= '<th class="stockAttributesHeadingComboId" title="This number is the Product ID and related Attributes (Unique Combo).">' . PWA_PAC . '</th>';
        $html .= '<th class="stockAttributesHeadingVariant">' . PWA_VARIANT . '</th>';
        $html .= '<th class="stockAttributesHeadingQuantity">' . PWA_QUANTITY_IN_STOCK . '</th>';
        $html .= '<th class="stockAttributesHeadingSort">' . PWA_SORT_ORDER . '</th>';
        $html .= '<th class="stockAttributesHeadingCustomid" title="The Custom ID MUST be Unique, no duplicates allowed!">' . PWA_CUSTOM_ID . '</th>';
        $html .= '<th class="stockAttributesHeadingSKUTitleId">' . PWA_SKU_TITLE . '</th>';
        $html .= '<th class="stockAttributesHeadingEdit">' . PWA_EDIT . '</th>';
        $html .= '<th class="stockAttributesHeadingDelete">' . PWA_DELETE . '</th>';
        $html .= '</tr>';
        $html .= '</thead>';
        $html .= '<tbody>';
        ...
        foreach ($attribute_products as $attribute_product) {
          ...
          $html .= '<tr id="sid-' . $attribute_product['stock_id'] . '">';
          $html .= '<td class="stockAttributesCellStockId">' . $attribute_product['stock_id'] . '</td>' . "\n";
          $html .= '<td>' . $attribute_product['product_attribute_combo'] . '</td>' . "\n";
          ...
          $sort2_query = "SELECT DISTINCT pa.products_attributes_id, po.products_options_sort_order, po.products_options_name
                          FROM " . TABLE_PRODUCTS_ATTRIBUTES . " pa
                          LEFT JOIN " . TABLE_PRODUCTS_OPTIONS . " po ON (po.products_options_id = pa.options_id)
                          WHERE pa.products_attributes_id IN (" . $attribute_product['stock_attributes'] . ")
                          " . $options_order_by;
          $sort_class = $db->Execute($sort2_query);
          $array_temp_sorted_array = array();
          $attributes_of_stock = array();
          foreach ($sort_class as $sort) {
            $attributes_of_stock[] = $sort['products_attributes_id'];
          }
          ...
          foreach ($attributes_of_stock as $attri_id) {
            ...
            if ($stock_attribute['option'] == '' && $stock_attribute['value'] == '') {
              ...
              $db->Execute("DELETE FROM " . TABLE_PRODUCTS_WITH_ATTRIBUTES_STOCK . " WHERE stock_id = " . $attribute_product['stock_id'] . " LIMIT 1;");
            } else {
...
//                  sort($attributes_output);
          ...
          $html .= '</td>' . "\n";
          $html .= '<td class="stockAttributesCellQuantity editthis" id="stockid-quantity-' . $attribute_product['stock_id'] . '">' . $attribute_product['quantity'] . '</td>' . "\n";
          $html .= '<td class="stockAttributesCellSort editthis" id="stockid-sort-' . $attribute_product['stock_id'] . '">' . $attribute_product['sort'] . '</td>' . "\n";
          $html .= '<td class="stockAttributesCellCustomid editthis" id="stockid-customid-' . $attribute_product['stock_id'] . '">' . $attribute_product['customid'] . '</td>' . "\n";
          $html .= '<td class="stockAttributesCellTitle" id="stockid-title-' . $attribute_product['stock_id'] . '">' . $attribute_product['title'] . '</td>' . "\n";
          $html .= '<td class="stockAttributesCellEdit">' . "\n";
          $html .= '<a href="' . zen_href_link(FILENAME_PRODUCTS_WITH_ATTRIBUTES_STOCK, "action=edit&amp;products_id=" . $product['products_id'] . '&amp;attributes=' . $attribute_product['stock_attributes'] . '&amp;q=' . $attribute_product['quantity'] . '&amp;search_order_by=' . $search_order_by, 'NONSSL') . '">' . PWA_EDIT_QUANTITY . '</a>'; //s_mack:prefill_quantity
          $html .= '</td>' . "\n";
          $html .= '<td class="stockAttributesCellDelete">' . "\n";
          $html .= '<a href="' . zen_href_link(FILENAME_PRODUCTS_WITH_ATTRIBUTES_STOCK, "action=delete&amp;products_id=" . $product['products_id'] . '&amp;attributes=' . $attribute_product['stock_attributes'] . '&amp;search_order_by=' . $search_order_by, 'NONSSL') . '">' . PWA_DELETE_VARIANT . '</a>';
          $html .= '</td>' . "\n";
          $html .= '</tr>' . "\n";
        }
        $html .= '</tbody>';
    ...
    $html .= '</tbody>';
    $html .= '</table>' . "\n";
    $html .= '<button type="submit" class="btn btn-primary btn-sm">' . IMAGE_SAVE . '</button>';
    $html .= '</form>' . "\n";
    if (!isset($_GET['seachPID']) && !isset($_GET['pwas-search-button']) && !isset($_GET['updateReturnedPID'])) {
      $html .= '<table class="table pageResults">';
      $html .= '<tr>';
...
//Update attribute qty
  function updateAttribQty($stock_id = null, $quantity = null)
  {
    ...
    if (!empty($stock_id) && is_numeric($stock_id) && is_numeric($quantity)) {
      $query = "UPDATE `" . TABLE_PRODUCTS_WITH_ATTRIBUTES_STOCK . "
  ...
  function insertNewAttribQty($products_id = null, $productAttributeCombo = null, $strAttributes = null, $quantity = 0, $customid = null, $skuTitle = null)
  {
    ...
    if (isset($products_id) && is_numeric($products_id) && isset($strAttributes) && is_numeric($quantity)) {
      ...
      $query = "SELECT count(*) AS total
                FROM " . TABLE_PRODUCTS_WITH_ATTRIBUTES_STOCK . "
                WHERE (products_id = " . (int)$products_id . "
                AND stock_attributes = '" . $strAttributes . "')
                " . (isset($productAttributeCombo) ? "OR product_attribute_combo = '" . $productAttributeCombo . "'" : "") . "
                " . (isset($customid) /* @TODO if customid is not required to be unique then and with a false */ ? "OR customid = :customid:" : "");
      if (!isset($customid)) {
      ...
      if ($result_keys->fields['total'] == 0) {
        ...
        $query = "INSERT INTO " . TABLE_PRODUCTS_WITH_ATTRIBUTES_STOCK . " (products_id, product_attribute_combo, stock_attributes, quantity, customid, title)
                  VALUES (" . (int)$products_id . ", :product_attribute_combo:, '" . $strAttributes . "', " . $quantity . ", :customid:, :title:)";
        ...
        if (isset($customid)) {
          $query = "SELECT count(*) AS total
                    FROM " . TABLE_PRODUCTS_WITH_ATTRIBUTES_STOCK . "
                    WHERE customid = '" . $customid . "':customid:";
          ...
          if ($result_customid->fields['total'] > 0) {
            ...
            $query = "SELECT count(*) AS total
                      ...
                      WHERE ((products_id = " . (int)$products_id . "
                        AND stock_attributes = '" . $strAttributes . "')
                        " . (isset($productAttributeCombo) ? "OR product_attribute_combo = :product_attribute_combo: " : "") . ")
  ...
  function insertTablePASR($products_id = null, $strAttributes = null, $quantity = null, $customid = null)
  {
    ...
    if (is_numeric($products_id) && isset($strAttributes)) {
      ...
      $query = "INSERT INTO " . TABLE_PRODUCTS_ATTRIBUTES_STOCK_RELATIONSHIP . " (products_id, products_attributes_id, products_attributes_stock_id)
                VALUES (" . (int)$products_id . ", " . $strAttributes . ", " . $pasid . ")
                ON DUPLICATE KEY UPDATE
                  products_id = " . (int)$products_id . ",
                  products_attributes_id = " . (int)$strAttributes;
      $result = $db->Execute($query);
...
//The on duplicate updates an existing record instead of adding a new one
  function insertTablePAS($products_id = null, $quantity = null, $customid = null)
  {
    ...
    global $db;
    //$stock = $products_with_attributes_stock_class;
    $customid = $db->prepare_input($customid);
    $customid = $this->nullDataEntry($customid); //sets proper quoting for input
//   INSERT INTO `znc_products_attributes_stock` (`products_id`, `quantity`, `customid`) VALUES (226, 636, 'test37');
    ...
    if (is_numeric($products_id)) {
      ...
      $query = "INSERT INTO " . TABLE_PRODUCTS_ATTRIBUTES_STOCK . " (products_id, quantity, customid)
                VALUES (" . (int)$products_id . ", " . $quantity . ", '" . $customid . "')
                ON DUPLICATE KEY UPDATE
                  products_id = " . (int)$products_id . ",
                  quantity = " . $quantity . ",
                  customid = '" . $customid . "'";
...
//Update  sku Title of Attribute using the StockID as a key
  function updateTitleAttrib($stockid = null, $skuTitle = null)
  {
    global $db;
    //$stock = $products_with_attributes_stock_class;
    $skuTitle = $db->prepare_input($skuTitle);
    $skuTitle = $this->nullDataEntry($skuTitle); //sets proper quoting for input
...
//note the $Field input (field name) is required to get returned data if name is not set or if there is not a $providedQuery.
  function selectItemID($Table, $Field, $current = null, $providedQuery = null, $name = null, $id = null, $class = null, $style = null, $onChange = null)
  {
    ...
    foreach ($results as $result) {
      ...
      $rowID = $result['products_id'];
      $Item = 'ID:&nbsp;&nbsp;' . $result['products_id'];
      $Item .= '&nbsp;&nbsp;Model:&nbsp;&nbsp;' . $result['products_model'];
      $Item .= '&nbsp;&nbsp;Name:&nbsp;&nbsp;' . $result['products_name'];

      if (($Item == $current && $current != NULL) || ($rowID == $current && $current != NULL)) {
      ...
      if (!empty($attributes) && is_array($attributes)) {
        ...
        if ($attribute_stock->RecordCount() > 0) {
          ...
          $query = "SELECT products_attributes_id
                    FROM " . TABLE_PRODUCTS_ATTRIBUTES . "
                    WHERE options_values_id IN ('" . implode(',', zen_db_prepare_input($attributes)) . "')
                    AND products_id = " . (int)$products_id . "
                    ORDER BY products_attributes_id;";
          $attributes_new = $db->Execute($query);

          foreach ($attributes_new as $item) {
            $stock_attributes[] = $item['products_attributes_id'];
          }
        ...
        $customid_query = "SELECT customid AS products_model
                           FROM " . TABLE_PRODUCTS_WITH_ATTRIBUTES_STOCK . "
                           WHERE products_id = " . (int)$products_id . "
                           AND stock_attributes IN ('" . $stock_attributes_comb . "')";
...
// This must match the structure of your products_attributes table
        ...
        foreach ($attributes_copy_from_ids as $key => $value) {
          ...
          if (array_key_exists($attributes_copy_from_ids_data_query->fields['options_id'], $attributes_copy_from_ids_data)) {
            ...
            if (is_array($attributes_copy_from_ids_data[$attributes_copy_from_ids_data_query->fields['options_id']]) && !empty($attributes_copy_from_ids_data[$attributes_copy_from_ids_data_query->fields['options_id']])) {
...
// Return a product ID with attributes as provided on the store front
  ...
  function convertDropdownsToSBA()
  {
    ...
    if (defined('PRODUCTS_OPTIONS_TYPE_SELECT_SBA')) {
      $sql = "UPDATE " . TABLE_PRODUCTS_OPTIONS . "
              SET products_options_type = :products_options_type_select_sba:
